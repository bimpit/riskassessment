...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: existing } = await srAny.from('assessments').select('*').eq('id', id).eq('team_id', teamId).single()
    const { data, error } = await srAny.from('assessments').update(body).eq('id', id).eq('team_id', teamId).select().single()
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await srAny.from('risks').delete().eq('assessment_id', id).eq('team_id', teamId)
    const { error } = await srAny.from('assessments').delete().eq('id', id).eq('team_id', teamId)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
